...
// yk: we had to move this definition up to satisfy compilation-order
// constraints. When merging upstream, it helps to put this class back where it
// was temporarily, do the merge, then move it back here. Otherwise it's very
// hard to see what our local changes really are.
class StackMaps {
public:
  struct Location {
  ...
  static unsigned getNextMetaArgIdx(const MachineInstr *MI,
                                             unsigned CurIdx);
...
public:
  ...
  unsigned getVarIdx() const {
    ...
    // Due to yk stackmap changes, finding the index isn't as simple as
    // before. We have to scan until we've seen enough NextLive markers.
