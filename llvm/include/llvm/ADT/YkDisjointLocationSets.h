...
class DisjointLocationSets {
  ...
  //
  // You should assume that calling this invalidates existing iterators to
  // `this.Sets` and iterators over any set containing `Loc`.
  ...
  //
  // Note that `this.Sets` shouldn't contain sets with only one element, so
  // if calling this creates such a set, it is the responsibility of the
  // caller to ensure eventual consistency.
  //
  // You should assume that any iterators over `this.Sets` are invalidated.
...
public:
  ...
  // sets.
  //
  // You should assume that this invalidates iterators over `this.Sets` as well
  // as iterators over the parent set of `Loc`.
  ...
  // Update the set containing `Loc` in `this.Sets` by intersecting it with
  // the set containing `Loc` in `Other.Sets`.
  //
  // Returns whether the operation changed the set in `this.Sets`.
  bool intersectSetsContaining(int64_t Loc, DisjointLocationSets &Other) {
    std::vector<std::set<int64_t>>::iterator BeforeIt = findInSet(Loc);
    std::set<int64_t> BeforeSet;
    if (BeforeIt != notFound()) {
      BeforeSet = *BeforeIt;
    } else {
      BeforeSet = {Loc};
    }

    std::vector<std::set<int64_t>>::iterator OtherIt = Other.findInSet(Loc);
    std::set<int64_t> OtherSet;
    if (OtherIt != Other.notFound()) {
      OtherSet = *OtherIt;
    } else {
      OtherSet = {Loc};
    }

    std::set<int64_t> InterSet;
    std::set_intersection(BeforeSet.begin(), BeforeSet.end(), OtherSet.begin(),
                          OtherSet.end(),
                          std::inserter(InterSet, InterSet.begin()));
    updateSet(Loc, InterSet);
    assert(verify());
    return BeforeSet != OtherSet;
  }
  ...
  bool verify() {
    ...
    for (auto &S : Sets) {
      // No set should have only one member. Instead the set should be absent
      // from `this.Sets`.
      if (S.size() <= 1) {
        return false;
      }
